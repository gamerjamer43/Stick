...
- Java levels of runtime optimization
...
- **Basic Compiler Opts** — There's probably about a hundred glaring optimizations I can make during compile time to make it much less of a hassle on startup, wind down, or on hot loops. A small excerpt: <p style="margin-bottom:5px;"></p>
    - Constant Folding. If there's an expression that can reliably evaluate to an integer constant (i.e. int x = 2 + 3) it will be "folded" (more or less solved and THEN constant pooled, i.e. int x = 6) to avoid the extra operation [DONE]
...
- **Green Threads** — OS level threads simply are not as well scheduled as green threads through Java or Go. My plan is to basically implement a scheduler that multiprocesses on each physical thread. That way, thousands of green threads can be used for I/O heavy tasks by switching context and locking relatively often, whereas if we just want to implement a few the context switching and locking is much fewer and far between.
